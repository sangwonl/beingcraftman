...
[[nodiscard]] Mat4<T> lookAt(const Vec3<T>& eye,
    ...
    // 회전 + 이동을 하나의 행렬로
    Mat4<T> m{};
    m[0] = Vec4<T>{ right.x,   up.x,   forward.x, 0 };
    m[1] = Vec4<T>{ right.y,   up.y,   forward.y, 0 };
    m[2] = Vec4<T>{ right.z,   up.z,   forward.z, 0 };
    m[3] = Vec4<T>{
        -dot(right, eye),
        -dot(up, eye),
        -dot(forward, eye),
...
TEST_CASE("Quat fromAxisAngle → toMat4 일관성") {
    ...
    for (int c = 0; c < 4; ++c)
        for (int r = 0; r < 4; ++r)
            CHECK(mq[c][r] == doctest::Approx(mr[c][r]).epsilon(EPS));
}
...
TEST_CASE("Quat slerp 중간점") {
    ...
    for (int c = 0; c < 4; ++c)
        for (int r = 0; r < 4; ++r)
            CHECK(m[c][r] == doctest::Approx(expected[c][r]).epsilon(EPS));
}
