...
int main() {
  ...
  // fragment shader
  unsigned int fragmentSahder = glCreateShader(GL_FRAGMENT_SHADER);
  glShaderSource(fragmentSahder, 1, &fragmentShaderSource, NULL);
  glCompileShader(fragmentSahder);
  // check for shader compile errors
  glGetShaderiv(fragmentSahder, GL_COMPILE_STATUS, &success);
  if (!success) {
    glGetShaderInfoLog(fragmentSahder, 512, NULL, infoLog);
  ...
  // modify this VAO, but this rarely happens. Modifying other VAOs requries a
  // call to glBindVertexArray always so we generally don't unbind VAOs (nor
