...
std::vector<double> derivative_central(const std::vector<TX> &x, const std::vector<TY> &y)
{
...
template <typename TX, typename TY>
std::vector<double> derivative(const std::vector<TX> &x, const std::vector<TY> &y)
{
    std::vector<double> derivatives;
...
std::string to_string(const std::vector<double>& vec)
{
    auto transformed = vec 
        | std::views::transform([](double v) { return std::to_string(v); });
